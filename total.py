...
insult = ['fyou', 'a*', 'f*', 'maul', 'schnauze', 'klappe', 'h*']
musicoff = ['off','aus']
musicon = ['an','on']
...
def unbekannt(cocktail):
    text = "Sorry not sorry, das kann ich leider noch nicht zubereiten. "
    print(">>>"+text) 
    mainSpeaking(text)
    wunsch()
...
#beleidigungen
def affront():
    text = "Keine Kraftausdrücke ihr Räuber"
    mainSpeaking(text)
...
def main():
    ...
    timeout = time.time() + 60*20  #*5 für 5 Minuten
